...
-- DEFINIR LA REGLA ASIGNADA A v_query: UM CON --> materiales con peso escurrido en donde no es KG su unidad de medida.
SET v_query = '''
    SELECT
        COUNT(*)
    FROM `cf-esproapro-bic-pro-ou.SH_STG.bqt_material_attr`
    WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND INHAL <> 0 AND (INHME IS NULL OR INHME != 'KG')
''';
...
SET v_total = (SELECT
  COUNT(*)
  FROM `cf-esproapro-bic-pro-ou.SH_STG.bqt_material_attr`
WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND INHAL <> 0);
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Coherencia en la unidad de medida del contenido');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_38_MARA_',
