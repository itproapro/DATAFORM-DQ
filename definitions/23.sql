--query_posicionamiento_ZZCLASIFICACION
...
-- DEFINIR LA REGLA ASIGNADA A v_query: Indicador de posicionamiento: 'BÁSICO','VENTAJOSO','EXCLUSIVO'.
...
SET v_query = '''
    ...
    WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND (ZZCLASIFICACION IS NULL OR ZZCLASIFICACION NOT IN ('BÁSICO','VENTAJOSO','EXCLUSIVO'));
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validación indicador de posicionamiento');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_23_MARA_',
...
WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND (ZZCLASIFICACION IS NULL OR ZZCLASIFICACION NOT IN ('BÁSICO','VENTAJOSO','EXCLUSIVO'))
GROUP BY MATNR, ZZCLASIFICACION ORDER BY MATNR ASC;
""", file_name);
