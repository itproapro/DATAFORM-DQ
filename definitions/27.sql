...
-- DEFINIR LA REGLA ASIGNADA A v_query: -- CÓDIGO EAN --> este campo no debería ser nulo.
SET v_query = '''
  SELECT
    COUNT(*)
  FROM `cf-esproapro-bic-pro-ou.SH_STG.bqt_material_attr` 
WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND EAN11 IS NULL
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Identificar códigos EAN nulos.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_27_MARA_',
