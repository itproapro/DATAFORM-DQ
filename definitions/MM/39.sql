...
-- DEFINIR LA REGLA ASIGNADA A v_query: SUJETO A LOTE --> se indica con una 'X' que los materiales se gestionan a lote.
SET v_query = '''
    SELECT
        COUNT(*)
    FROM `cf-esproapro-bic-pro-ou.SH_STG.bqt_material_attr` 
    WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND XCHPF != 'X';
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validación de la gestión del material por lote.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_39_MARA_',
