...
-- DEFINIR LA REGLA ASIGNADA A v_query: GPO.TOLERANCIA --> Indicador del porcentaje de tolerancia en materiales catch weight.
...
SET v_query = '''
        ...
        AND
        (CWM_TOLGR IS NOT NULL OR CWM_TOLGR != '')
        AND CWM_XCWMAT != 'X';
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Coherencia en el campo de porcentaje de tolerancia para materiales catch weight.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_68_MARA_',
