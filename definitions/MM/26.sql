...
-- DEFINIR LA REGLA ASIGNADA A v_query: -- Indicador FRAGIL: este campo debería estar indicado para todos los materiales de pastelería. 
SET v_query = '''
  SELECT
    COUNT(*)
  FROM `cf-esproapro-bic-pro-ou.SH_STG.bqt_material_attr`
  WHERE MTART IN ('ZMER','ZSEC','ZFRE') 
    AND LEFT(LPAD(CAST(PRDHA AS STRING),18,'0'),12) = '101010010510'
    AND (ZZFRAGIL IS NULL OR ZZFRAGIL != 'X');
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validación del campo indicador frágil');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_26_MARA_',
...
EXECUTE IMMEDIATE FORMAT("""
  ...
  AS
    SELECT
      ...
      LEFT(LPAD(CAST(PRDHA AS STRING),18,'0'),12) AS PRDHA,
      ZZFRAGIL,
      MTART,
    FROM `cf-esproapro-bic-pro-ou.SH_STG.bqt_material_attr`
