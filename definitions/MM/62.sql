...
-- DEFINIR LA REGLA ASIGNADA A v_query: CLAVE HORIZONTE --> verificamos que el campo tenga un valor igual a '000'.
...
SET v_query = '''
        ...
        ON m.MATNR = c.MATNR
        WHERE m.MTART IN ('ZMER','ZSEC','ZFRE') AND (FHORI IS NULL OR FHORI != '000');
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validamos la clave que calcula los tiempos de holgura necesarios para la programación de una orden.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_62_MARA_',
