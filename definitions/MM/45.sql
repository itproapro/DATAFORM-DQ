--query_pais_origen_producto_HERKL
...
-- DEFINIR LA REGLA ASIGNADA A v_query: PAIS ORIGEN --> verificamos que el país origen del producto no sea nulo.
SET v_query = '''
        SELECT
            COUNT(*),
        ...
        ON m.MATNR = c.MATNR
        WHERE m.MTART IN ('ZMER','ZSEC','ZFRE') AND HERKL IS NULL;
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validación del país de origen del material.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_45_MARA_',
...
EXECUTE IMMEDIATE FORMAT("""
  ...
  AS
    ...
    JOIN `cf-esproapro-bic-pro-ou.SH_STG.bqt_mat_plant_attr` AS c 
    ON m.MATNR = c.MATNR
    WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND HERKL IS NULL;
""", file_name);
