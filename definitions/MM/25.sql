...
-- DEFINIR LA REGLA ASIGNADA A v_query: -- UMP_VAR--> este campo indica 1 cuando la unidad de medida base es UNIDAD. 
SET v_query = '''
  WITH base AS(
SELECT
   MATNR,
...
eval AS(
  SELECT
    *,
    CASE
      WHEN MTART IN ('ZMER','ZSEC','ZFRE') AND VABME = '2' THEN 1
      WHEN MTART IN ('ZMER','ZSEC','ZFRE') AND MEINS = 'ST' AND VABME IS NULL THEN 1
      ELSE NULL
    END AS incorrectos
  FROM base
)
 SELECT
  COUNTIF(incorrectos = 1) AS materiales_incorrectos FROM eval;
''';
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_25_MARA_',
