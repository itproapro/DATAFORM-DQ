...
-- DEFINIR LA REGLA ASIGNADA A v_query: PESO ESCURRIDO--> este campo indica el peso escurrido del material, y su unidad de medida es KG.
...
WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND ((INHAL >= BRGEW) OR (INHAL >= NTGEW) OR (INHME IS NULL AND INHAL <> 0));
''';
...
SET v_total = (SELECT
  ...
  FROM `cf-esproapro-bic-pro-ou.SH_STG.bqt_material_attr`  
WHERE 
( MTART IN ('ZMER','ZSEC','ZFRE') AND INHAL <> 0));
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validación del peso escurrido');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_24_MARA_',
