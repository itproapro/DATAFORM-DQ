...
-- DEFINIR LA REGLA ASIGNADA A v_query: -- ZZMARCA--> este campo indica la marca del material.
SET v_query = '''
  SELECT 
    COUNT(*) FROM `cf-esproapro-bic-pro-ou.SH_STG.bqt_material_attr`
  WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND ZZMARCA IS NULL;
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, 'Validación Materiales sin Marca detallada:');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_22_MARA_',
