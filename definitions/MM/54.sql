--query_planif.necesidades_DISPO
...
-- DEFINIR LA REGLA ASIGNADA A v_query: PLANIF.NECESIDADES --> verificamos que el valor del campo sea igual a 'Z01'
SET v_query = '''
    SELECT
        COUNT(*)
    ...
    ON m.MATNR = c.MATNR
    WHERE m.MTART IN ('ZMER','ZSEC','ZFRE') AND (DISPO IS NULL OR DISPO != 'Z01');
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validación del número del planificador de necesidades.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_54_MARA_',
