...
-- DEFINIR LA REGLA ASIGNADA A v_query: TMPO. TRATAMIENTO EM-->  verificamos los valores que son diferentes a 1.
...
SET v_query = '''
        ...
        ON m.MATNR = c.MATNR
        WHERE m.MTART IN ('ZMER','ZSEC','ZFRE') AND WEBAZ != 1;
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Verifica cantidad de días necesarios para verificar y almacenar el material después de recibir la mercancía.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_60_MARA_',
