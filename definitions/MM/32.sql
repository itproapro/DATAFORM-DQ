...
-- DEFINIR LA REGLA ASIGNADA A v_query: VOLUMEN --> Por el momento solo comprobamos aquellos materiales en donde figura el volumen igual a 0 o nulo. 
SET v_query = '''
    SELECT
    COUNT(*)
FROM `cf-esproapro-bic-pro-ou.SH_STG.bqt_material_attr` 
WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND (VOLUM = 0 OR VOLUM IS NULL);
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validación del campo volumen de los materiales');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_32_MARA_',
