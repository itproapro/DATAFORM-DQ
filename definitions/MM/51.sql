...
-- DEFINIR LA REGLA ASIGNADA A v_query: UM.PED.COMPRA--> verificamos que el campo cumpla la siguiente relación:
-- si UMB = UN => UM.PED.COMPRA = CJ.
SET v_query = '''
        SELECT
            COUNT(*)
        ...
        ON m.MATNR = c.MATNR
        WHERE m.MTART IN ('ZMER','ZSEC','ZFRE') AND BSTME IS NULL AND MEINS = 'ST'
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Coherencia entre UMB = ST y unidad de pedido de compra igual a CJ.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_51_MARA_',
...
EXECUTE IMMEDIATE FORMAT("""
  ...
  AS 
    ...
    ON m.MATNR = c.MATNR
    WHERE m.MTART IN ('ZMER','ZSEC','ZFRE') AND BSTME IS NULL AND MEINS = 'ST'
    GROUP BY MATNR,WERKS, BSTME, MEINS;
""", file_name);
