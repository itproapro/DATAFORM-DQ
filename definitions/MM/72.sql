...
-- DEFINIR LA REGLA ASIGNADA A v_query: CODIGO_EAN_UN_2 -> Código ean11 (código ean unidad interior)
...
SET v_query = '''
        ...
        ON m.MATNR = mr.MATNR 
        JOIN `cf-esproapro-bic-pro-ou.SH_STG.bqt_mat_sales_attr` AS ms
        ON m.MATNR = ms.MATNR
        WHERE
            (MTART IN ('ZMER','ZFRE', 'ZSEC'))
            AND
            (mr.MEINH = 'CS' AND (mr.EAN11 IS NULL OR mr.EAN11 =''))
            OR
            (ms.MEGRU = 'Z001' AND MEINS = 'ST' AND mr.MEINH = 'ST' AND (mr.EAN11 IS NULL OR mr.EAN11 =''))
            OR
            (ms.MEGRU IN ('Z002', 'Z001') AND mr.MEINH = 'CS' AND (mr.EAN11 IS NULL OR mr.EAN11 =''));
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validamos el campo EAN de la unidad en aquellos casos que sea posible la venta por UN.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_72_MARA_',
