...
-- DEFINIR LA REGLA ASIGNADA A v_query: -- Cantidad Unidad de medida base 5 --> verificamos que PAL sea múltiplo de CAP y que CAP sea múltiplo de CS.
...
SET v_query = '''
    WITH MARM AS (
        ...
        WHERE
            ...
            OR
            --CAP debe ser múltiplo de CS.
            (
            umrez_cap IS NOT NULL
            AND umrez_cj IS NOT NULL
            AND MOD(umrez_cap,umrez_cj)!= 0
            );
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validamos el valor de la cantidad de unidad de medida base 5 (CAP) en relación con la cantidad de umb4(PAL)  y en realción a can_umb2 (CJ) o si umb=UN será en relación a can_uma2(UN).');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_85_MARA_',
