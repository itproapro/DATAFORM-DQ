...
-- DEFINIR LA REGLA ASIGNADA A v_query: GRUPO CARGA --> verificamos que el valor de este campo sea nulo o distinto de '0001'.
SET v_query = '''
        SELECT
            COUNT(m.MATNR)
        ...
        ON m.MATNR = c.MATNR
        WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND (LADGR != '0001' OR LADGR IS NULL);
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validación del grupo de carga.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_42_MARA_',
