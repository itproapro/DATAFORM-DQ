--query_indicador_margen_seguridad_SHFLG
...
-- DEFINIR LA REGLA ASIGNADA A v_query: IND. MARGEN SEGURIDAD --> indicador que posibilita activar el margen de seguridad/ cobertura real para un material en la planificación de necesidades.
...
SET v_query = '''
    ...
    ON m.MATNR = c.MATNR
    WHERE m.MTART IN ('ZMER','ZSEC','ZFRE') AND (SHFLG != '2' OR SHFLG IS NULL) AND (WERKS IN ('6440','6441','4300','6442','4400','6340','6341','6342','6242'))
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validamos el indicador que activa el margen de seguridad.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_63_MARA_',
