...
-- DEFINIR LA REGLA ASIGNADA A v_query: REGION ORIGEN--> verificamos que el campo tenga un valor constante igual a 08
SET v_query = '''
        SELECT
            COUNT(*)
        ...
        ON m.MATNR = c.MATNR
        WHERE m.MTART IN ('ZMER','ZSEC','ZFRE') AND (HERKR != '08' OR HERKR IS NULL)
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validación del campo región.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_47_MARA_',
...
EXECUTE IMMEDIATE FORMAT("""
  ...
  AS
    ...
    JOIN `cf-esproapro-bic-pro-ou.SH_STG.bqt_mat_plant_attr` AS c 
    ON m.MATNR = c.MATNR
    WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND HERKR IS NULL OR HERKR != '08'
    GROUP BY MATNR,HERKR,WERKS, MTART; 
""", file_name);
