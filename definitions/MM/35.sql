...
-- DEFINIR LA REGLA ASIGNADA A v_query: ANCHO --> por el momento la única validación es que este campo no sea nulo ni igual a 0.
SET v_query = '''
    SELECT
        COUNT(*)
    FROM `cf-esproapro-bic-pro-ou.SH_STG.bqt_material_attr` 
    WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND (BREIT = 0 OR BREIT IS NULL);
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validamos el dato logístico: ancho');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_35_MARA_',
...
EXECUTE IMMEDIATE FORMAT("""
  ...
  AS
    SELECT
        RIGHT(LPAD(CAST(MATNR AS STRING),18,'0'),5),
        MEINS,
        BREIT,
        MEABM
    FROM `cf-esproapro-bic-pro-ou.SH_STG.bqt_material_attr`
    WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND MEINS = 'CS' AND (BREIT = 0 OR BREIT IS NULL);
""", file_name);
