...
-- DEFINIR LA REGLA ASIGNADA A v_query: GR PL NEC --> verificamos el total de materiales con un valor de GR PL NEC distinto a '3060' o nulo.
SET v_query = '''
    SELECT
        COUNT(*)
    ...
    ON m.MATNR = c.MATNR
    WHERE m.MTART IN ('ZMER','ZSEC','ZFRE') AND (DISGR IS NULL OR DISGR != '3060');
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validación grupo planificación de necesidades.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_53_MARA_',
