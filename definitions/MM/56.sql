--query_clase_aprovisionamiento_BESKZ
...
-- DEFINIR LA REGLA ASIGNADA A v_query: CLASE APROVISIONAMIENTO--> verificamos que el valor del campo sea igual a 'F'
SET v_query = '''
        SELECT
            COUNT(*)
        ...
        ON m.MATNR = c.MATNR
        WHERE m.MTART IN ('ZMER','ZSEC','ZFRE') AND BESKZ != 'F'
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validación del tipo de aprovisionamiento, en nuestro caso compra externo.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_56_MARA_',
