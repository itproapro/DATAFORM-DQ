...
-- DEFINIR LA REGLA ASIGNADA A v_query: TAMAÑO LOTE --> verificamos que el valor del campo sea igual a 'WB' o 'EX'
SET v_query = '''
    WITH base AS (
        SELECT
        ...
        SELECT
            ...
            WERKS,
            CASE
                WHEN DISLS = 'WB' THEN 'OK'
                WHEN DISLS = 'EX' THEN 'OK'
                ELSE 'INCORRECTOS'
            END AS resultado
        FROM base
        )
        SELECT
            COUNTIF(resultado != 'OK' OR resultado = 'REVISAR') AS incorrectos,
        FROM evaluacion
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validación de la clave que determina el cálculo del tamaño del lote fijando la cantidad de aprovisionamiento. ');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_55_MARA_',
  ...
  AS 
    WITH base AS (
        ...
        SELECT
            *
        FROM evaluacion
        WHERE MTART IN ('ZMER','ZFRE','ZSEC') AND resultado = 'INCORRECTOS';
""", file_name);
