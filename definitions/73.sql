--query_tipo_ean_un2_EANTP_NUMTP
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validamos la relación entre código EAN y el tipo de código EAN.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_73_MARA_',
...
EXECUTE IMMEDIATE FORMAT("""
  ...
  AS   
    SELECT
      ...
      mr.EAN11,
      mr.NUMTP,
      LENGTH(mr.EAN11) AS longitud_ean
    FROM `cf-esproapro-bic-pro-ou.SH_STG.bqt_material_attr` AS m
    JOIN `cf-esproapro-bic-pro-ou.SH_STG.bqt_ztbw_marm` AS mr
    ON m.MATNR = mr.MATNR
      WHERE
