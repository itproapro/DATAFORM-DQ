...
-- DEFINIR LA REGLA ASIGNADA A v_query: CAN_UMB_2 --> Cantidad unidad de medida base 2.
...
SET v_query = '''
        ...
        AND
        (MEINS = 'CS' AND mr.MEINH = 'CS' AND (UMREZ IS NULL OR UMREZ != 1))
        OR
        (MEINS = 'ST' AND mr.MEINH = 'CS' AND (UMREZ IS NULL OR UMREZ < 1));
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validamos el campo de conversión de cantidad de unidades según el valor de UMB.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_71_MARA_',
