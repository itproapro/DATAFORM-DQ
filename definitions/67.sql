...
-- DEFINIR LA REGLA ASIGNADA A v_query: IND.PERIODO --> Indicador que especifica en qué intervalos se llevan los valores de consumo y pronóstico del material.
...
SET v_query = '''
        ...
        ON m.MATNR = c.MATNR
        WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND  (PERKZ != 'T' OR PERKZ IS NULL);
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validamos el indicador que especifíca en que intervalos se consume y pronostica el material.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_67_MARA_',
