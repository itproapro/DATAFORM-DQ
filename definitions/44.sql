...
-- DEFINIR LA REGLA ASIGNADA A v_query: TARIC --> verificamos que el campo no sea nulo y que la longitud este entre 6 y 10 dígitos.
SET v_query = '''
        SELECT
            COUNT(*)
        ...
        ON m.MATNR = c.MATNR
        WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND (STAWN IS NULL OR LENGTH(STAWN) NOT BETWEEN 6 AND 10);
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validación de formato de partida arancelaria (TARIC).');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_44_MARA_',
