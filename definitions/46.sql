...
-- DEFINIR LA REGLA ASIGNADA A v_query: GRUPO EXPORT--> verificamos que el campo tenga un valor constante igual a 0001
SET v_query = '''
        SELECT
            COUNT(*)
        ...
        ON m.MATNR = c.MATNR
        WHERE m.MTART IN ('ZMER','ZSEC','ZFRE') AND (MTVER IS NULL OR MTVER != '0001')
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validación del grupo de requerimientos para export / import.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_46_MARA_',
...
EXECUTE IMMEDIATE FORMAT("""
  ...
  AS
    ...
    JOIN `cf-esproapro-bic-pro-ou.SH_STG.bqt_mat_plant_attr` AS c 
    ON m.MATNR = c.MATNR
    WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND (MTVER IS NULL OR MTVER != '0001')
    GROUP BY MTVER, MATNR, MTART, WERKS;
""", file_name);
