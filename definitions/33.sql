...
-- DEFINIR LA REGLA ASIGNADA A v_query: UNIDAD DE VOLUMEN --> validamos los materiales que no tiene M3 como unidad de volumen. 
SET v_query = '''
    SELECT
        COUNT(*)
    FROM `cf-esproapro-bic-pro-ou.SH_STG.bqt_material_attr` 
    WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND VOLEH IS NULL OR VOLEH != 'M3';
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Coherencia en el valor de unidad de volumen');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_33_MARA_',
