...
-- DEFINIR LA REGLA ASIGNADA A v_query: UN. DIMENSION --> por el momento la única validación es que este campo no sea nulo.
SET v_query = '''
    SELECT
        COUNT(*)
    FROM `cf-esproapro-bic-pro-ou.SH_STG.bqt_material_attr` 
    WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND (MEABM IS NULL OR MEABM != 'MM');
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Coherencia en la unidad de dimensión');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_37_MARA_',
