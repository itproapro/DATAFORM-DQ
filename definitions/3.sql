...
-- DEFINIR LA REGLA ASIGNADA A v_query: TIPO DE MATERIAL según MATNR
SET v_query = '''
    WITH base AS (
  SELECT
    MATNR,
    MTART,
    LEFT(RIGHT(CAST(MATNR AS STRING),5),1)AS d5
...
eval AS (
  SELECT 
    *,
     CASE
      WHEN d5 = '1' AND MTART = 'ZMER' THEN 1
      WHEN d5 = '3' AND MTART = 'ZFRE' THEN 1
      WHEN d5 = '4' AND MTART = 'ZSEC' THEN 1
      WHEN d5 NOT IN ('1', '3', '4') AND MTART IN ('ZMER','ZFRE','ZSEC') THEN 0
      ELSE NULL
     END AS cumple
  FROM base
)
SELECT 
  COUNTIF(cumple = 0) AS materiales_incorrectos
FROM eval;
''';
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_3_MARA_',
