...
-- DEFINIR LA REGLA ASIGNADA A v_query: -- UNIDAD DE PESO --> Esta regla, comprueba que nuestra unidad de peso por defecto es el KG.
SET v_query = '''
SELECT
    COUNT(GEWEI)
FROM `cf-esproapro-bic-pro-ou.SH_STG.bqt_material_attr` 
WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND GEWEI != 'KG'
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, 'Comprobar si unidad por defecto es KG');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_31_MARA_',
