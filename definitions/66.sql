--query_leer.material.prov_TEMPB
...
-- DEFINIR LA REGLA ASIGNADA A v_query: LEER.MAT.PROV. --> verificamos que el campo tenga un valor constante igual a 'X'.
...
SET v_query = '''
        ...
        ON m.MATNR = c.MATNR
        WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND  TEMPB IS NULL;
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validamos el indicador para las condiciones de temperatura para un material.');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_66_MARA_',
