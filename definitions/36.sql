...
-- DEFINIR LA REGLA ASIGNADA A v_query: ALTO --> por el momento la única validación es que este campo no sea nulo ni igual a 0.
SET v_query = '''
    SELECT
        COUNT(*)
    FROM `cf-esproapro-bic-pro-ou.SH_STG.bqt_material_attr` 
    WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND (HOEHE = 0 OR HOEHE IS NULL);
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validamos el dato logístico: alto');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_36_MARA_',
...
EXECUTE IMMEDIATE FORMAT("""
  ...
  AS
    SELECT
        ...
        MEABM
    FROM `cf-esproapro-bic-pro-ou.SH_STG.bqt_material_attr`
    WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND MEINS = 'CS' AND (HOEHE = 0 OR HOEHE IS NULL);
""", file_name);
