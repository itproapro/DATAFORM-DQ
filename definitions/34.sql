...
-- DEFINIR LA REGLA ASIGNADA A v_query: LARGO --> por el momento la única validación es que este campo no sea nulo ni igual a 0. 
SET v_query = '''
    SELECT
        COUNT(*)
    FROM `cf-esproapro-bic-pro-ou.SH_STG.bqt_material_attr` 
    WHERE MTART IN ('ZMER','ZSEC','ZFRE') AND MEINS = 'CS' AND (LAENG = 0 OR LAENG IS NULL);
''';
...
SET v_details = CONCAT('Total: ', v_total, ', Failed: ', v_failed, ' Validamos el dato logístico: largo');
...
SET file_name = CONCAT(
  'gs://maestromateriales-dataquality-pap/REGLA_DQ_34_MARA_',
